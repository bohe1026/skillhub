          ...
          <div className="space-y-1">
            ...
            <p className="font-semibold">
              {review.status === 'PENDING' && (
                <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-700 text-sm">{t('review.statusPending')}</span>
              )}
              {review.status === 'APPROVED' && (
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-700 text-sm">{t('review.statusApproved')}</span>
              )}
              {review.status === 'REJECTED' && (
                <span className="px-2.5 py-0.5 rounded-full bg-red-500/10 text-red-700 text-sm">{t('review.statusRejected')}</span>
              )}
            </p>
          </div>
          <div className="space-y-1">
