...
class SkillPublishServiceTest {
    ...
    @Test
    void testPublishFromEntries_ShouldDeleteReviewTasksBeforeReplacingRejectedVersion() throws Exception {
        String namespaceSlug = "test-ns";
        String publisherId = "user-100";
        String skillMdContent = "---\nname: test-skill\ndescription: Test\nversion: 1.0.0\n---\nBody";
        ...
        assertEquals("1.0.0", result.version().getVersion());
        assertEquals(SkillVersionStatus.PENDING_REVIEW, result.version().getStatus());
        InOrder inOrder = inOrder(reviewTaskRepository, skillVersionRepository);
        inOrder.verify(reviewTaskRepository).deleteBySkillVersionIdIn(List.of(8L));
        inOrder.verify(skillVersionRepository).delete(rejectedVersion);
    }
